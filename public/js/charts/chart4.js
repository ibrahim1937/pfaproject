    ...
    function showGraph2(x,y){
        var myPieChart = new Chart(ctx, {
            ...
            data: {
                ...
                datasets: [{
                data: x,
                backgroundColor: ['#4e73df', '#1cc88a', '#36b9cc',"#90EE90","#acdd6c"],
                hoverBackgroundColor: ['#2e59d9', '#17a673', '#2c9faf',"#90EE90","#acdd6c"],
...
function remplir(selector, myData) {
    ...
    for (let i = 0; i < myData.length; i++) {
        ligne += "<tr><td> " + myData[i].elem + "</td>";
        ligne += "<td> " + myData[i].nom + "</td>";
        ligne += "<td> " + myData[i].prenom + "</td>";
        ligne += "<td> " + myData[i].created_at + "</td>";
        ligne += "<td> " + (myData[i].etat == 'prête' ? 'acceptée' : myData[i].etat) + "</td></tr>";
