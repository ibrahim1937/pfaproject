$(document).ready(function(){
        ...
        $.ajax({
            ...
            success: function(data){
                if(data.message.title == 'fail'){
                    $('#add').prepend('<div class="alert alert-danger m-3 fail">'+ data.message.message +'</div>');
                } else if(data.message.title == 'success'){
                    $('#add').prepend('<div class="alert alert-success m-3 success">'+ data.message.message +'</div>');
                }
    ...
    $('#delete').click(function(e){
        ...
        var message = checkedcount() == 1 ? msg + 'cette filiere ? ' : msg + checkedcount() + ' filieres ?';
    ...
    $(document).on('click', '.modifier', function(e){
        e.preventDefault();
        $('#codem').val($(this).parent().closest('tr').find('td').eq(0).text());
        $('#libellem').val($(this).parent().closest('tr').find('td').eq(1).text());
        sessionStorage.setItem('idfiliere', parseInt($(this).parent().closest('tr').find('th').eq(0).text()));
    ...
    $('#modifier').click(function(e){
        ...
        if($('#codem').val() && $('#libellem').val()){
            ...
            $('.errorparent').prepend('<div class="alert alert-danger m-3 fail">Check your entry</div>');
            hideMessage();
        }
...
function remplir(selector, myData){
    ...
    for(let i=0; i<myData.length; i++){
        ligne+= '<tr><th scope="row"><input type="checkbox" name="filieres" value=""> &nbsp '+ myData[i].id + '</th>';
        ligne+= '<td> ' + myData[i].code + '</td>';
        ligne+= '<td> ' + myData[i].libelle + '</td>';
        ligne+= '<td class="text-center"><button type="button" class="btn btn-primary modifier" title="Modifier une filiere" data-bs-toggle="modal" data-bs-target="#exampleModal">Modifier</button></td></tr>';
